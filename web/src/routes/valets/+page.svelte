<script lang="ts">
	...
	import WorkflowDiagram from '$lib/components/valets/WorkflowDiagram.svelte';
	import AlertsBanner from '$lib/components/valets/AlertsBanner.svelte';
	import CostBreakdown from '$lib/components/valets/CostBreakdown.svelte';

	let refreshInterval: number | null = null;

	// Enhanced state
	let pipeline = $state<PipelineStatusResponse | null>(null);
	let costs = $state<CostMetricsResponse | null>(null);
	...
	async function fetchEnhancedData() {
		try {
			const [pipelineData, costsData, alertsData] = await Promise.all([
				getPipelineStatus(),
				getCostMetrics(valetsStore.selectedPeriod),
				getValetAlerts()
			]);
			...
			console.error('[Valets] Enhanced data fetch error:', err);
		}
	}
	...
	function handlePeriodChange(period: 'today' | '7d' | '30d') {
		...
		getCostMetrics(period).then((data) => (costs = data));
	}

	function handleValetClick(valetName: string) {
		haptic('light');
		goto(`/valets/${valetName}`);
	}
...
<div class="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
	...
	<!-- Alerts Banner -->
	{#if showAlerts && alerts && alerts.alerts.length > 0}
		<AlertsBanner
			alerts={alerts.alerts}
			totalCritical={alerts.total_critical}
			totalWarning={alerts.total_warning}
			totalInfo={alerts.total_info}
					...
					<div class="flex items-center gap-2">
						<span
							class={`w-3 h-3 rounded-full ${
								valetsStore.systemStatus === 'healthy'
									? 'bg-green-500'
									: valetsStore.systemStatus === 'degraded'
										? 'bg-yellow-500'
										: 'bg-red-500'
							} ${valetsStore.systemStatus === 'healthy' ? 'animate-pulse' : ''}`}
						></span>
						<span
							class={`font-semibold ${valetsStore.getSystemStatusColor(valetsStore.systemStatus)}`}
						>
							{valetsStore.systemStatus === 'healthy'
	...
	{#if pipeline}
		<WorkflowDiagram
			...
			totalInPipeline={pipeline.total_in_pipeline}
			estimatedMinutes={pipeline.estimated_completion_minutes}
			bottleneckValet={pipeline.bottleneck_valet}
		/>
	{:else if valetsStore.loading}
		<Skeleton variant="rectangular" class="h-36 rounded-xl" />
	{/if}
				...
				<button class="text-left w-full" onclick={() => handleValetClick(valet.name)}>
					...
					>
						...
						<div class="flex items-start justify-between mb-3">
							...
							<span
								class={`px-2 py-0.5 text-xs rounded-full ${valetsStore.getStatusBgColor(valet.status)} ${valetsStore.getStatusColor(valet.status)}`}
							>
						...
						{#if valet.current_task}
							<div class="mb-3 p-2 rounded-lg glass-subtle">
								...
								<p class="text-sm text-[var(--color-text-primary)] line-clamp-2">
									{valet.current_task}
								</p>
							</div>
						{/if}
						...
						<div class="grid grid-cols-2 gap-2 text-center">
							<div class="p-2 rounded-lg glass-subtle">
								<p class="text-lg font-semibold text-[var(--color-text-primary)]">
									{valet.tasks_completed_today}
								</p>
								<p class="text-xs text-[var(--color-text-tertiary)]">Tâches</p>
							</div>
							<div class="p-2 rounded-lg glass-subtle">
								<p
									class="text-lg font-semibold {valet.error_count_today > 0
										? 'text-red-400'
				...
				<Card class="p-4 h-full">
					...
					<div class="overflow-x-auto">
						<table class="w-full text-sm">
							<thead>
								<tr
									class="text-left text-[var(--color-text-tertiary)] border-b border-[var(--glass-border-subtle)]"
								>
									...
									<tr class="border-b border-[var(--glass-border-subtle)] last:border-0">
										<td class="py-3 pr-4">
											<button
												...
												onclick={() => handleValetClick(metric.name)}
											>
										...
										<td class="py-3 pr-4 text-right {metric.tasks_failed > 0 ? 'text-red-400' : ''}"
											>{metric.tasks_failed}</td
										>
										<td class="py-3 pr-4 text-right">{metric.avg_duration_ms}ms</td>
										<td class="py-3 pr-4 text-right">
											<span
												class={metric.success_rate >= 0.95
													? 'text-green-400'
							...
							<tfoot>
								<tr class="font-semibold text-[var(--color-text-primary)]">
									...
									<td class="py-3 pr-4 text-right">-</td>
									<td class="py-3 pr-4 text-right"
										>{valetsStore.metrics.total_tokens.toLocaleString()}</td
									>
								</tr>
							</tfoot>
						</table>
