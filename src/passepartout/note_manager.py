...
class NoteManager:
    ...
    def add_strategic_question(
        ...
        """
        Add a strategic question to a note's "Questions ouvertes" section.

        Strategic questions are generated by the valets during email analysis
        and require human reflection/decision.

        Format in the note:
        ### ❓ {question}
        - **Catégorie** : {category}
        - **Source** : {source_valet} (via email "{source_email_subject}")
        - **Contexte** : {context}
        - **Ajoutée le** : {date}

        Args:
            note_id: Note identifier
            question: The strategic question text
            category: Category (decision, processus, organisation, structure_pkm)
            source_valet: Which valet generated the question (grimaud, bazin, etc.)
            context: Context explaining why this question matters
            source_email_subject: Subject of the source email for traceability

        Returns:
            True if added successfully, False if note not found
        """
        note = self.get_note(note_id)
        if not note:
            logger.warning(
                "Note not found for add_strategic_question",
                extra={"note_id": note_id, "question": question[:50]},
            )
            return False

        # Format the question entry
        date = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        # Truncate email subject for readability
        email_subject_short = (
            source_email_subject[:50] + "..."
            if len(source_email_subject) > 50
            else source_email_subject
        )

        entry = f"""### ❓ {question}
- **Catégorie** : {category}
- **Source** : {source_valet} (via email "{email_subject_short}")
- **Contexte** : {context}
- **Ajoutée le** : {date}
"""

        # Find or create "Questions ouvertes" section
        content = note.content
        section_header = "## Questions ouvertes"
        ...
        if section_header in content:
            ...
            for line in lines:
                ...
                if line.strip() == section_header and not entry_added:
                    ...
                    # Add entry after an empty line following the header
                    if line.strip() == "":
                        new_lines.append(entry)
                        entry_added = True
                    elif line.startswith("### "):
                        # Another question exists, insert before it
                        new_lines.insert(len(new_lines) - 1, entry)
                        entry_added = True

            # If section was found but empty
            if section_found and not entry_added:
                for i, line in enumerate(new_lines):
                    if line.strip() == section_header:
                        new_lines.insert(i + 1, "")
        ...
        success = self.update_note(note_id, content=content)

        if success:
            # Update metadata to reflect pending question
            self._increment_questions_count(note_id)
            logger.info(
                "Added strategic question to note",
                extra={
                    "note_id": note_id,
    ...
    def _increment_questions_count(self, note_id: str) -> None:
        """Increment questions count and set pending flag in metadata."""
        try:
            from src.passepartout.note_metadata import get_metadata_store

            store = get_metadata_store()
            metadata = store.get_metadata(note_id)
            if metadata:
    ...
    def _decrement_questions_count(self, note_id: str) -> None:
        """Decrement questions count and update pending flag in metadata."""
        try:
            from src.passepartout.note_metadata import get_metadata_store

            store = get_metadata_store()
            metadata = store.get_metadata(note_id)
            if metadata and metadata.questions_count > 0:
                metadata.questions_count -= 1
                metadata.questions_pending = metadata.questions_count > 0
                store.save_metadata(metadata)
        except Exception as e:
            logger.warning(f"Failed to update questions metadata for {note_id}: {e}")
