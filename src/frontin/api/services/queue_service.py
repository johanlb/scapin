...
class QueueService:
    ...
    async def approve_item(
        ...
        # v3.2: Distribute strategic questions to target notes
        self._distribute_strategic_questions(item)
    ...
    def _distribute_strategic_questions(self, item: dict[str, Any]) -> dict[str, list[str]]:
        """
        Distribute strategic questions to their target notes.

        Strategic questions are generated by the valets during analysis.
        When an item is processed (approved/rejected/modified), questions
        with a target_note are added to that note's "Questions ouvertes" section.
        Questions without target_note are stored as orphan questions for the briefing.

        Args:
            item: Queue item with analysis containing strategic_questions

        Returns:
            Dict with 'distributed' (note IDs where questions were added)
            and 'orphaned' (question IDs stored in orphan storage)
        """
        ...
        result: dict[str, list[str]] = {"distributed": [], "orphaned": []}

        # Get strategic questions from analysis
        analysis = item.get("analysis", {})
        ...
        # Get email subject and item ID for traceability
        metadata = item.get("metadata", {})
        email_subject = metadata.get("subject", "Email inconnu")
        item_id = item.get("id", "unknown")

        note_manager = get_note_manager()
        orphan_storage = get_orphan_questions_storage()
        ...
        for sq in strategic_questions:
            ...
            source_valet = sq.get("source", "mousqueton")

            if not question:
                continue

            if target_note:
                # Add to the target note
                success = note_manager.add_strategic_question(
                    note_id=target_note,
                    question=question,
                    category=category,
                ...
                if success:
                    result["distributed"].append(target_note)
                    logger.info(
                        "Distributed strategic question to note",
                        extra={
                            "note_id": target_note,
                    ...
                    # Target note not found - store as orphan
                    orphan = orphan_storage.add_question(
                        question=question,
                        category=category,
                        context=context,
                        source_valet=source_valet,
                        ...
                        intended_target=target_note,
                    )
                    result["orphaned"].append(orphan.question_id)
                    logger.warning(
                        "Target note not found, stored as orphan question",
                        extra={
                            "note_id": target_note,
                            "question": question[:50],
                            "orphan_id": orphan.question_id,
                        },
                    )
            else:
                # No target note - store as orphan for briefing
                orphan = orphan_storage.add_question(
                    question=question,
                    category=category,
                    context=context,
        ...
        # Log summary
        if result["distributed"] or result["orphaned"]:
            logger.info(
                "Strategic questions distribution complete",
                extra={
                    "item_id": item_id,
                    "distributed_count": len(result["distributed"]),
    ...
    def _build_multi_pass_metadata(self, result: Any) -> dict[str, Any] | None:
        ...
        # v3.2: Include canevas status
        canevas_status = None
        if hasattr(result, "canevas_status") and result.canevas_status:
            canevas_status = result.canevas_status
